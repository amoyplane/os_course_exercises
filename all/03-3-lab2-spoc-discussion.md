...
访问数据时要check目标数据段的DPL，控制转移时一般是低特权调用高特权；中断异常时判断门和代码特权级。

2. 在什么情况下会出现特权级切换？

在ring0和ring3间切换时，低特权调用高特权代码，或者中断发生。

3. int指令在ring0和ring3的执行行为有什么不同？

压栈内容不同，在ring3时多压栈ss:ESP。同时要切换栈。

4. 如何利用int和iret指令完成不同特权级的切换？

人工压栈并修改栈内参数，再用int和iret指令切换。

5. TSS和Task Register的作用是什么？

TSS：保存任务状态信息的系统段。task register拥有当前任务的TSS的段选择符和段描述符。
...
取指令一次，指令里三个参数可能要三次，发生4次地址转换。

2. 描述X86-32的MMU地址转换过程；

先找到对应段表的页表，经过多级页表获得页号，加上偏移得到物理地址。
