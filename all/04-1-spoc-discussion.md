...
均衡繁忙充分利用了各个硬件，没有显著短板。

3. 在你写程序时遇到过内存不够的情况吗？尝试过什么解决方法？

用过，存储数据库的时候。尝试用文件做缓存解决。

### 8.2 覆盖和交换

1. 什么是覆盖技术？使用覆盖技术的程序开发者的主要工作是什么？

指的是程序按功能和调用关系分块，不会同时执行的模块共享一块内存。程序员负责模块划分和模块关系确定。

2. 什么是交换技术？覆盖与交换有什么不同？

程序整体换入换出。不需要划分模块，解决进程所需空间不足的问题。
...
时间：某时刻访问了存储器的某一位置，可能在较短时间内再次访问同一位置；  
空间：某时刻访问了存储器的某一位置，可能在较短时间内再次访问附近位置；  
分支：同一个分支的两次选择大概率相同；  
